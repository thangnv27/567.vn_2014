<?php
/*
Module Name: Categories Banners
Description: Categories Banners module allow you to add an image to category or any custom term. Build the based Zahlan's Categories Images plugin v2.3.1.
Version: 1.0.0
Author: Ngo Van Thang
Author URI: http://ngovanthang.info/
*/

define('Z_IMAGE_PLACEHOLDER', get_template_directory_uri() . "/includes/categories-banners/images/placeholder.png");

// l10n
load_plugin_textdomain('zci', FALSE, 'categories-banners/languages');
...
function z_add_texonomy_field() {
    ...
    echo '<div class="form-field">
		<label for="taxonomy_image">' . __('Banner', 'zci') . '</label>
		<textarea name="taxonomy_image" id="taxonomy_image"></textarea>
		<!--<br/>
...
function z_edit_texonomy_field($taxonomy) {
    ...
    echo '<tr class="form-field">
            <th scope="row" valign="top"><label for="taxonomy_image">' . __('Banner', 'zci') . '</label></th>
            <td>
                <div>' . z_taxonomy_image_url($taxonomy->term_id, FALSE) . '</div>
                <textarea name="taxonomy_image" id="taxonomy_image">' . $image_text . '</textarea><br />
		<!--<button class="z_upload_image_button button">' . __('Upload/Add image', 'zci') . '</button>
...
function z_options_menu() {
    ...
    add_submenu_page($menuname, __('Categories Images settings', 'zci'), __('Categories Images', 'zci'), 'manage_options', 'zci-options', 'zci_options');
